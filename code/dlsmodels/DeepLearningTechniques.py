...
class DeepLearningTechniques (BaseModel):
    ...
    def get_main_metric_objective (self):
        """
        Returns the main metric objective
        """
        return 'min' if self.dataset.get_task_type () == 'regression' else 'max'
        
    
    def get_path (self, key):
        """
        Returns a path from a list of predefined routes
        ...
        # @var paths 
        paths = {
            'folder': self.dataset.get_working_dir (task, 'models', model_folder),
            'models': self.dataset.get_working_dir (task, 'models'),
            'hyperparameters': self.dataset.get_working_dir (task, 'models', model_folder, 'hyperparameters.csv'),
            'trials': self.dataset.get_working_dir (task, 'models', model_folder, 'trial-'),
            'resume': self.dataset.get_working_dir (task, 'models', model_folder, 'training_resume.json'),
            'logs': self.dataset.get_working_dir (task, 'models', model_folder, 'logs'),
            'word_embeddings_tokenizer': self.dataset.get_working_dir (task, 'we_vectorizer.pickle'),
            'best_model': self.dataset.get_working_dir (task, 'models', model_folder, 'model.h5')
        }
    ...
    def train (self, force = False):
        ...
        # Save memory. 
        # We only keep the information we need in the dataframe
        # to perform the training. This is useful as RayTune makes
        # copies of the data
        df = df[['__split', 'label']]
        self.dataset.df = df
        
        
        # @var task_type string Determine the task type (regression, classification, ...)
        task_type = self.dataset.get_task_type ()
        ...
        # If the problem is classification, then we need to encode the label as 
        # numbers instead of using names
        ...
        if 'we' in self.features:
            ...
            self.features['we'].load_vectorizer_from_disk (self.get_path ('word_embeddings_tokenizer')) 
            
            
            # @var tokenizer Tokenizer
            tokenizer = self.features['we'].get_vectorizer ()
        ...
        best_model_file = self.get_path ('best_model')
        ...
        shutil.rmtree (self.get_path ('folder'), ignore_errors = True)
        ...
        scheduler = ray.tune.schedulers.AsyncHyperBandScheduler (time_attr = 'training_iteration',
            metric = main_metric,
            mode = self.get_main_metric_objective (),
            grace_period = 10
        )
        ...
        # @var train_resume Dict 
        # We track information of the training process
        ...
        # Attach data for classification
        if task_type in ['classification']:
        ...
        def ray_train (config, dataset, features, x, y, class_weights, reporter = None, checkpoint_dir = None, data_dir = None):
            """
            Encapsulates how RAY should train a model
            ...
            def get_learning_rate_scheduler_callback (lr, epochs):
                """
                Implementation of a time-based decay
            ...
            def get_y_pred (model, X):
                """
                Returns the predictions of a model for classification 
            ...
            def get_y_real (tensorflow_dataset):
                """
                Returns the real labels of a dataset
                
                @param tensorflow_dataset
                """
            ...
            def get_early_stopping_metric ():
                """
                Returns the early stopping metric
                """
            ...
            def get_patience_per_achitecture (architecture, features):
                """
                Returns the patience, based on the architecture
            ...
            if 'regression' == task_type:
                ...
                # @var y_real List
                y_real = np.concatenate ([y for x, y in datasets['train']], axis = 0)
                ...
                tune.report (
                    loss = history.history['loss'][-1],
                    val_loss = history.history['val_loss'][-1],
                    rmse = mean_squared_error (y_true = y_real, y_pred = model.predict (datasets['train'], batch_size = len (train_df))),
                    val_rmse = mean_squared_error (y_true = y_val, y_pred = model.predict (datasets['val'], batch_size = len (val_df)))
                )
        ...
        # Prepare the parameters for evauation
        for group in parameters_to_evaluate:
        ...
        search_alg = ray.tune.suggest.hyperopt.HyperOptSearch (search_space,
            metric = main_metric, 
            mode = self.get_main_metric_objective (),
            random_state_seed = bootstrap.seed
        )
        ...
        class GetBestTrialCallback (Callback):
            ...
            def on_trial_result (self, iteration, trials, trial, result, **info):
                if 'done' in result:
                    ...
                    if 'rmse' in result:
                        self.hyperparameter_df_handler.update_metric.remote (trial.trial_id, 'rmse', result['rmse'])
                        self.hyperparameter_df_handler.update_metric.remote (trial.trial_id, 'val_rmse', result['val_rmse'])
        ...
        analysis = tune.run ( 
            ...
            local_dir = self.get_path ('models'),
            name = model_folder,
        ...
        # @var best_run_id int
        if task_type == 'regression':
            best_run_id = hyperparameter_df['objective'].idxmin ()
        ...
        hyperparameter_df.to_csv (self.get_path ('hyperparameters'), index = False)
        
        
        # Store the training resume
        with open (self.get_path ('resume'), 'w') as resume_file:
    ...
    def get_best_model (self, use_train_val = False):
        """
        @inherit
        """
    ...
    def predict (self, callback = None):
        ...
        # @var hyperparameter_df DataFrame 
        hyperparameter_df = pd.read_csv (self.get_path ('hyperparameters'))
