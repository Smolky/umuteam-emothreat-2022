...
class BertModel (BaseModel):
    ...
    def compute_metrics (self, pred):
        ...
        if 'multi_label' == task_type:
            preds = torch.as_tensor ((preds - .5) > 0, dtype = torch.int32) 
            labels = torch.as_tensor ((labels - .5) > 0, dtype = torch.int32)
    ...
    def train (self, force = False):
        ...
        train_resume['pretrained_model'] = train_resume['pretrained_model'].replace (config.directories['assets'], '../assets')
