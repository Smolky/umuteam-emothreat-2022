...
class SentenceEmbeddingsTransformer (BaseEstimator, TransformerMixin):
    ...
    This load sentence embeddings from fastText
    
    Uses FastText sentence embeddings
    """
    
    def __init__ (self, model, cache_file = '', field = ''):
        """
        @param model String
        @param field String
    ...
    def transform (self, X, **transform_params):
        ...
        if self.cache_file and os.path.exists (self.cache_file):
            return pd.read_csv (self.cache_file, header = 0, sep = ',', float_precision = 'round_trip').astype ('float64')
    ...
    def get_feature_names (self):
        return [self.prefix + '_' + str (x) for x in range (1, 301)]
