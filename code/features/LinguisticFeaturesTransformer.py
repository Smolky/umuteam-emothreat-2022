...
class LinguisticFeaturesTransformer (BaseEstimator, TransformerMixin):
    ...
    def __init__ (self, model = 'default.xml', cache_file = '', field = '', label = 'label'):
        """
    ...
    def transform (self, X, **transform_params):
        ...
        X[self.field] = X[self.field].str.replace (r'\\', r'\\\\', regex = True)
        ...
        def get_lf_batch (X):
            ...
            with tempfile.NamedTemporaryFile () as temp:
                ...
                save_progress_file = (self.prefix + '-' + str (X.index[0]) + '-' + str(X.index[-1]) + '.csv') if not is_fake_data and self.prefix else ''
                
                
                # IF the file exists, it was already computed
                if save_progress_file and os.path.exists (self.temp_folder + save_progress_file):
                    return pd.read_csv (self.temp_folder + save_progress_file, header = 0, sep = ",")
            ...
            if save_progress_file:
                features.to_csv (self.temp_folder + save_progress_file, index = False)
