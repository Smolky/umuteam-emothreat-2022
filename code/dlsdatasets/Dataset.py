...
class Dataset ():
    ...
    def finetune_df (self, df):
        ...
        if 'label' in df.columns:
            ...
            if 'regression' == self.get_task_type ():
    ...
    def get_vector_labels (self, df):
        """
        Returns a vector of categories for classification and multi-label
        categories
        ...
        if 'regression' == task_type:
            ...
            return df.dropna (subset = ['label'])['label']
        
        # Classification
        if 'classification' == task_type:
        
            # @var group_results_by_user Boolean
            group_results_by_user = self.group_results_by_user ()        
            
            
            # Get the results grouped by user according to the task
            if group_results_by_user:
                return df.groupby (by = 'user', sort = False)['label'].apply (lambda x: x.mode().iloc[0])
            else:
                return df['label']
        ...
        if 'multi_label' == task_type:
            ...
            # @var df_labels Series
            df_labels = df['label'].astype ('category')
            df_labels = df_labels.cat.add_categories ('none')
            df_labels.fillna ('none', inplace = True)
            
            
            # @var y_real List
            y_real = df_labels.str.split ('; ')
